...
public abstract class BaseDAO extends
     ...
     * 根据条件分页查询数据
     *
     * @param statementName sql操作语句，一般从iBATIS配置文件中，以命名空间形式得到
     * @param paramMap      条件map 键对应数据库字段
     * @param pageNum       当前页码
...
//    }
     ...
     *
     * @param sql  命名SQL的ID
     * @param para 参数列表,执行的次数为para.size()次，每次执行时候的参数迭代list中每个Object元素
     */
     ...
     *
     * @param sql  命名SQL的ID
     * @param para 参数列表,执行的次数为para.size()次，每次执行时候的参数迭代list中每个Object[]元素
     */
     ...
     * 提供结果集的行处理
     *
     * @param namedSqlId 命名SQL的IDiBATIS文件中配置
     * @param para       条件参数
     * @param rowHandler
     */
     ...
     *
     * @param namedSqlId 命名SQL的IDiBATIS文件中配置
     * @param para       条件参数，如果为空则所有，如果有值则查询满足条件的所有
     * @return 结果集，List
     */
     ...
     * 返回单个记录的Object
     *
     * @param namedSqlId sql语句，iBATIS配置
     * @param para       条件
     * @return 返回某个满足条件的对象
     */
